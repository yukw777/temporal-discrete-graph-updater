...
from tdgu.preprocessor import BERTPreprocessor, SpacyPreprocessor
...
def test_BERT_preprocessor_preprocess(batch, expected_preprocessed, expected_mask):
    sp = BERTPreprocessor(["<pad>", "<unk>", "my", "name", "is", "peter"])
    preprocessed, mask = sp.preprocess(batch)
    assert preprocessed.equal(expected_preprocessed)
    assert mask.equal(expected_mask)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ["My name is Peter"],
            torch.tensor([[2026, 2171, 2003, 2848]]),
        ...
        (
            ["My name is Peter", "Is my name David?"],
            torch.tensor([[2026, 2171, 2003, 2848, 0], [2003, 2026, 2171, 2585, 1029]]),
            torch.tensor([[1, 1, 1, 1, 0], [1, 1, 1, 1, 1]]),
        ),
    ],
)
...
def test_BERT_preprocessor_load_from_file():
    ...
    sp_alt = BERTPreprocessor()
    assert len(sp.word_to_id_dict) != 0
    assert len(sp.word_to_id_dict) == len(sp_alt.word_to_id_dict)
...
def test_BERT_preprocessor_clean(raw_str, cleaned, batch_size):
    sp = BERTPreprocessor.load_from_file("vocabs/word_vocab.txt")
    assert sp.clean(raw_str) == cleaned
    assert sp.batch_clean([raw_str] * batch_size) == [cleaned] * batch_size
