"""Generate a smaller test dataset with the specified number of examples from
the given dataset."""
import argparse
import itertools
import json
...
# extract graphs
prev_seen_graph_idx = {str(e["previous_graph_seen"]) for e in examples}
graphs = {g_id: graph_index["graphs"][g_id] for g_id in prev_seen_graph_idx}

# extract relations
relation_idx = {str(r_id) for r_id in itertools.chain.from_iterable(graphs.values())}
